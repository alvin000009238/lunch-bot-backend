 ...
 * =================================================================
 * 1. 修正 CORS 跨域問題。
 * 2. 加入後台登入驗證功能 (/admin/login)。
 * 3. 適用於 Google Cloud Run 的啟動設定。
 * 4. 包含所有 LINE Bot 事件處理的完整邏輯。
 */
// --- 1. 引入需要的套件 ---
...
app.post('/admin/settings', async (req, res) => {
    ...
    try {
        const query = `INSERT INTO app_settings (key, value) VALUES ('deadline_time', $1) ON CONFLICT (key) DO UPDATE SET value = $1;`;
...
app.post('/admin/users/:id/deposit', async (req, res) => {
    ...
    try {
        ...
        const updateResult = await dbClient.query('UPDATE users SET balance = balance + $1 WHERE id = $2 RETURNING *', [amount, userId]);
        if (updateResult.rows.length === 0) throw new Error('找不到該使用者');
        const updatedUser = updateResult.rows[0];
        await dbClient.query('INSERT INTO transactions (user_id, type, amount) VALUES ($1, $2, $3)', [userId, 'deposit', amount]);
        await dbClient.query('COMMIT');
