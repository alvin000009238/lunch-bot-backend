...
async function handleFollowEvent(userId, replyToken) {
    try {
        ...
        const welcomeMessage = { type: 'text', text: `歡迎 ${profile.displayName}！您已成功註冊午餐訂餐服務，可以開始使用「菜單」指令囉！` };
...
async function handleOrderAction(userId, productId, orderForDate, replyToken) {
    ...
    try {
        ...
        if (userResult.rows.length === 0 || productResult.rows.length === 0) throw new Error('找不到使用者或產品');
        ...
        const orderInsertResult = await dbClient.query('INSERT INTO orders (user_id, total_amount, status, order_for_date) VALUES ($1, $2, $3, $4) RETURNING id', [user.id, product.price, 'preparing', orderForDate]);
        const orderId = orderInsertResult.rows[0].id;
        await dbClient.query('INSERT INTO order_items (order_id, product_id, quantity, price_per_item) VALUES ($1, $2, $3, $4)', [orderId, productId, 1, product.price]);
        ...
        await dbClient.query('INSERT INTO transactions (user_id, type, amount, related_order_id) VALUES ($1, $2, $3, $4)', [user.id, 'payment', product.price, orderId]);
        await dbClient.query('COMMIT');
        const successMessage = { type: 'text', text: `訂購「${product.name}」成功！\n用餐日期: ${orderForDate}\n訂單編號: ${orderId}\n消費金額: $${product.price}\n剩餘餘額: $${newBalance.toFixed(2)}` };
...
async function askForSupplier(replyToken, forDate) {
    try {
        ...
        const buttons = result.rows.map(supplier => ({ type: 'button', style: 'primary', height: 'sm', margin: 'sm', action: { type: 'postback', label: supplier.name, data: `action=select_supplier&date=${forDate}&supplierId=${supplier.id}`, displayText: `我想看 ${supplier.name} 的菜單` } }));
